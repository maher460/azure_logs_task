...
parquet_dir = "./parquet_data_concurrent"
...
def download_and_process_blob(blob, container, container_dir, locks):
    """
    Downloads and processes a single blob.
    
    Args:
    blob (Blob): The blob to download and process.
    container (ContainerClient): The Azure Blob Storage container client.
    container_dir (str): The directory to save Parquet and CSV files.
    locks (dict): A dictionary of locks for each date.
    """
    max_retries = 3
    ...
    df_data = pd.concat([pd.json_normalize(json.loads(blob_json)) for blob_json in blob_data.splitlines()], ignore_index=True)
    date = extract_date_from_name(blob.name)
    
    # Use a lock to synchronize access to df_logs for the specific date
    with locks[date]:
        df_logs = load_or_create_dataframe(container_dir, date)
        df_logs = pd.concat([df_logs, df_data], ignore_index=True)
        save_logs_to_disk(container_dir, date, df_logs)
...
def download_save_json_logs(container_uri, start_date=None, end_date=None, max_retries=3):
    ...
    try:
        ...
        # Use ThreadPoolExecutor to download and process blobs in parallel
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for blob in blob_list:
                ...
                if (((not start_date) or (start_date and int(date) >= int(start_date)))
                    ...
                    futures.append(executor.submit(download_and_process_blob, blob, container, os.path.join(parquet_dir, container_name), locks))
